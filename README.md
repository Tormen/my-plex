...
- **Bidirectional sync** between Plex metadata and filesystem markers
  ...
  - **Universal scope (v1.9)** — every SCOPE-taking command (`--list`, `--mv`, `--remux`, `--plex2disk`, `--disk2plex`, `--original-languages`, …) now shares one resolver. SCOPE can be a library name, cache key, full filepath, title search, or filter expression (`lang:fr`, `country:france`, `original_lang:de`, `year>2020`, `bitrate>2`, etc.). Multiple tokens **AND-combine**: e.g. `my-plex --mv movies.fr ,unsorted original_lang:fr` resolves to "every originally-French movie currently in `,unsorted`" and moves them. Identical token semantics across `--list` and every action command — write the same query once, plug it into the verb you want.
